...
fn describe_role(role: &SemanticRole) -> String {
    match role {
        ...
        SemanticRole::CarriedCap { side } => match side {
            CapSide::Start => "carried cap start".to_owned(),
            CapSide::End => "carried cap end".to_owned(),
            _ => "carried cap (unknown side)".to_owned(),
        },
