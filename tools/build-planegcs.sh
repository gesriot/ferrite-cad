...
#
# The same discipline as Open CASCADE, and for the same reason: planegcs is
# LGPL-2.0-or-later, so it is linked dynamically and the library this produces
# can be replaced by the user with their own build. Nothing of it is compiled
# into a FerriteCAD binary. See THIRD_PARTY_LICENSES.md.
#
...
eigen="${FCAD_EIGEN_INCLUDE:-/usr/include/eigen3}"
...
cp "${WORK}/FreeCAD-${FREECAD_TAG}/src/Mod/Sketcher/App/planegcs/GCS.h" /dev/null 2>/dev/null || true
...
files ./tree/SketcherGlobal.h and ./tree/Base/Console.h are FerriteCAD's own
build glue and are MIT.
NOTICE
