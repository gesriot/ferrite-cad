...
#
# Checks that every file FerriteCAD authors declares its licence.
#
...
while IFS= read -r file; do
    ...
    case "$file" in
        *.rs | *.cpp | *.hpp | *.h | *.yml | *.yaml | *.toml | *.sh | */CMakeLists.txt | CMakeLists.txt) ;;
        *) continue ;;
    esac
    ...
    if ! head -5 "$file" | grep -qF "$SPDX"; then
        missing+=("$file")
    fi
done < <(git ls-files)
