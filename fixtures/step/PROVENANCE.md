...
The files in `canonical` and `damaged` were generated by this repository. A
file written by Open CASCADE and read by Open CASCADE proves a round trip, not
that the reader copes with files written by another CAD system.

The `interoperability` directory answers that second question. Its inputs are
preserved byte for byte as received, and each one has its own provenance and
expected baseline below. They are not regenerated with the canonical corpus.
...
## The interoperability corpus

`c3d-ap203-complex-assembly.stp` was supplied by a FerriteCAD user on
2026-08-30 after `ferritecad import-step` refused it. Its header identifies C3D
