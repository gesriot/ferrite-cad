...
- **Source:** not delivered, and deliberately. The licence excepts copies "in
  the form of machine-executable object code generated by a source language
  processor" from carrying the notice, and the shared library is that. The
  version and digest are recorded in provenance anyway, and
  `LICENSE-Boost-BSL-1.0.txt` is copied out of the checked archive. Both are
  inventory and review decisions, not a claim that the exception requires
  them.
