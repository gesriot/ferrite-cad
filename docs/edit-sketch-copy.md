...
С §27F request строже: дубли JSON-ключей на любом уровне (в том числе
записанные через escape) и массив на месте объекта запроса или вершины
отказываются как `input` до открытия источника.
