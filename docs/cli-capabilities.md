...
Срез §24A. Это описание **текущих** команд и владельцев, а не проект нового протокола.
...
Не спроектированы и не приняты: новый протокол, JSON schema, DSL, RPC/MCP server, универсальный dispatcher, библиотека «всех операций». Не добавлены команды, флаги, UI, зависимости, renderer или изменения persistence. Будущие требования §4.5 (stdin, версия контракта вывода, отмена в CLI, пакетный ввод) ниже названы **требованиями**, а не существующим контрактом. В конце указан выбранный после ревью следующий срез §24B; его реализация ещё не начата.

## Карта текущих операций

| Предметный результат | UI сейчас | Публичный CLI сейчас | Общий API / владелец | Фактический пробел |
| --- | --- | --- | --- | --- |
| Пустой нативный `.fcad` | Нет. Пустое окно — отсутствие документа, не пустой файл. Нет New/Save. | `ferritecad create <path>` | [`Document::create_with`](../crates/ferritecad-document/src/document.rs) в `ferritecad-document`. Транзакция записи — [`Document::write`](../crates/ferritecad-document/src/document.rs). | UI не создаёт документы. У `create` нет `--force`: существующий путь всегда отказ. |
| Sample plate с шириной, глубиной, высотой | Нет. Моделирование в окне отсутствует. | `ferritecad create <path> --sample --size W D H` | Запись объектов — `Document::write`. Наполнение плиты — [`sample::populate`](../crates/ferritecad-cli/src/sample.rs), **только CLI-крейт**, не общая библиотека. Размеры — координаты эскиза и `Expression::constant` высоты, не объекты `Parameter`. | Нет правки уже созданной плиты. Нет общей операции, которую UI мог бы вызвать. Случайные `ObjectId` при каждом `create` не совпадают. |
...
Создать sample plate **не значит** произвольно редактировать параметры, эскизы и фичи существующей модели. `create --sample --size` задаёт прямоугольник и высоту **в момент создания**. Последующей команды изменить ширину, заменить сегмент, добавить отверстие, fillet или параметр нет. В документе плиты нет объектов `Parameter`: ширина и глубина — координаты четырёх линий эскиза, высота — константа `Blind` у `Extrude` ([`sample::populate`](../crates/ferritecad-cli/src/sample.rs)). Типы `Parameter` и `SketchConstraint` в `ferritecad-document` есть; пользовательской операции «задать параметр / поставить ограничение» нет.
...
- `create` пока пишет прямо по указанному пути. Его отказ на уже существующий файл проверен, но атомарность публикации через `Temporary` на эту команду не распространяется; см. известный дефект ниже.

### Чтение и возможная запись документа

`inspect`, `validate`, `dump-graph` и `clear-cache` используют `Document::open`, который может мигрировать старую схему и настраивает SQLite connection. Эти команды не обещают неизменность файла во всех случаях. Измерение неизменных байтов/mtime текущего документа не доказывает read-only контракт старых файлов. Viewer, `rebuild --cold`, `print-topology` и оба экспорта используют `open_read_only`: старую схему или WAL-состояние отказывают, а не мигрируют (*из кода*).
...
**Известный дефект `create`, измерен при ревью:** `ferritecad create bad.fcad --sample --size NaN 50 12` возвращает exit 2 (`model values must be finite`), но оставляет по новому пути пустой `.fcad` с нулём объектов. Создание файла предшествует `sample::populate`; откат `Document::write` не отменяет уже созданный файл. Поэтому для текущего `create` нельзя обещать «ошибка — нового файла нет». Исправление входит в следующий срез §24B; документация не считает этот дефект нормальным исходом создания плиты.
...
| Пробел | Статус |
| --- | --- |
| UI не создаёт и не правит модель; нет New/Save, нет STEP Open | факт |
| `sample::populate` живёт в CLI-крейте, не в общем слое | факт; первый кандидат на общую запись |
...
Независимое ревью воспроизвело оставленный пустой файл после отказа `create --sample --size NaN 50 12` (см. «Ошибка»). Его исправление включено в §24B вместе с общей операцией создания. `export-stl` на imported STEP ведёт себя согласно своему правилу выбора тел.

## Следующий ограниченный срез

**Выбран после независимого ревью; реализация не начата.** Для Claude Code, §24B.

Не строить универсальный dispatcher, JSON schema, RPC или «слой всех будущих команд».

Приблизить **одну** общую записывающую операцию и **два** её клиента:

1. Исправить воспроизведённый отказ создания с оставленным файлом: операция публикует готовый документ атомарно через существующий механизм `Temporary`, без замены существующего назначения. Транзакция SQLite и публикация нового файла — разные границы; нужны обе. Вынести создание пустого `.fcad` и sample plate (плоскость, прямоугольный профиль, extrude, хранимые cap/side refs, размеры в мм) из CLI-модуля в библиотечную транзакцию на `Document::write`. Публичная `ferritecad create` остаётся тем же CLI-клиентом без новых флагов и без смены контракта единиц/no-clobber.
2. Второй клиент — первое действие записи во вьюере: New → выбор пустого документа или плиты с размерами → место сохранения → создание `.fcad` → обычный Open результата. До сохранения нет принятого нового документа. Это ограниченный сценарий создания, без редактора эскиза, дерева параметров, STEP-из-UI и жестового моделирования.
3. Gate: оба клиента после сохранения дают тот же предметный результат — те же типы объектов, роли ссылок, `validate`, `rebuild --cold`, STL/FBX. Равенство UUID независимых созданий не требуется; FBX содержит эти identity, поэтому два новых документа не обязаны давать одинаковые FBX-байты. Сравнивать связи и геометрию с корректным сопоставлением идентификаторов, а два экспорта одного сохранённого документа — побайтово. Проверить отказ/отмену без публикации файла, сохранность старого документа и сцены. Тест общего пути без кликов и отдельно настоящий CLI-процесс.

Это закрывает разрыв «единственный нативный конструктор модели живёт в бинаре CLI» и ставит общую запись *до* первого свободного редактирования эскиза (§9) и волн фич (§10). Параметрическое изменение уже существующей плиты, импорт STEP из UI и протокол агента — следующие срезы, не этот.
