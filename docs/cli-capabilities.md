...
Срез §24A, обновлён срезами §24B–§24J и §25A–§25E. Это описание **текущих** команд и владельцев, а не проект нового протокола.
...
§25E добавляет отдельную сохраняемую H/V-правку ограниченного Line Sketch.
...
| H/V constraints сохранённого Line Sketch → новая копия | `Edit H/V …`, explicit segment list, Add H/V, Remove exact UUID, Save copy | `edit-sketch-constraints-copy source --sketch UUID --expect-version TOKEN --request constraints.json -o copy [--json]` | document managed H/V/closure policy + narrow writer → jobs `edit_sketch_constraints_copy`, тот же snapshot/cold/Keep lifecycle; UI/CLI только adapters | [Контракт и рецепт](sketch-constraints-copy.md). Stored coordinates не меняются, solves временны; H/V + adjacent Coincident, без arbitrary families/drag/in-place Save. |
