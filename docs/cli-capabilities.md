...
| Второй отдельный circular Cut в том же Body | Та же форма Add cut, с показом существующего инструмента и требуемого зазора | Та же `cut-circular-copy` и request v1 | Прежний shared copy job; qualified origin names через OCCT history, archive v3 | Ровно два отдельных диска, четыре through/pocket сочетания; первый Cut и SQL-данные сохраняются. [Контракт и рецепт](sequential-circular-cuts.md). |
| Правка сохранённого circular Cut | `Edit cut <имя> — <UUID>…`: сохранённые числа, Apply, Undo/Redo, Save Cancel и сохранность draft при отказах | `ferritecad edit-circular-cut source.fcad --feature UUID --expect-version HASH --request request.json -o copy.fcad [--json]` | `CircularCutEdit` → `prepare_cut_parameters` → узкий `write_cut_parameters` → прежний `edit_object_copy`; каталог из одного снимка | Один Cut или любой из двух над прямоугольной плитой (§26D). Прежние UUID/история/refs/SQL сохранены. Через→карман добавляет собственную ссылку на дно; для первого из двух также final OriginCap. Карман→через отказывает со всеми защищаемыми UUID. Discovery: `features[].circular_cut_edit`; прежний `editable` у Cut остаётся false. [Контракт и рецепт](edit-sequential-cuts.md). |
