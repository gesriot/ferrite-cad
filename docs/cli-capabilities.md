...
Срез §24A, обновлён срезом §24B. Это описание **текущих** команд и владельцев, а не проект нового протокола.
...
- UI на занятое назначение отвечает `already exists; choose a different file name` — своими словами и без флага, которого у окна нет. Замену окно **не** предлагает: общий `create` её не умеет.
...
Отказ и отмена создания (CLI или UI) нового `.fcad` не оставляют: ни по назначению, ни рядом с ним (§24B, **измерено**).
