...
Этот редактор остаётся редактором **чисел** сохранённой пары. Параметрическое
кольцо — радиусы, концентричность и закреплённый центр — добавил
[§25O](annular-circle-constraints.md) через общий constraint-маршрут; пока такие
ограничения на профиле есть, `annulus_edit` честно недоступен, и снова доступен,
когда они сняты.
