...
Исправлены пять дефектов продукта/тестов и пробел reader-кампании CI.
Локальные проверки ниже относятся к исправленному коду.
...
Аудит первого опубликованного head выявил ещё один пробел: CI создавал пять
новых FBX, но reader loop читал только два. В цикл добавлены centre/radius/depth;
его фактический Bash-блок исполнен локально на всех пяти файлах. Старый head
имел 41 reader execution на ОС, исправленный должен иметь 44; CI повторяется
на новом SHA, старый статус ему не приписывается. Обновление только скрипта
также обнаружило пропуск path filter runtime workflow: добавлены сам
check-fbx-complex.sh и scripts независимого reader, чтобы такое изменение
запускало кампанию автоматически.
