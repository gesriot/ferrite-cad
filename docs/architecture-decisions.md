...
- Все зависимости фиксируются в `THIRD_PARTY_LICENSES.md`; доступные тексты и
  notices включаются в поставку.
- Неустановленный текст, спорная трактовка или другая лицензионная серая зона
  записывается как известный риск, но не блокирует разработку, упаковку или
  релиз. Лицензионный результат `cargo-deny` является информационным.
- Проект не обращается к upstream и не ведёт дальнейшее проактивное
  исследование таких вопросов. Вернуться к ним можно только отдельным решением
  владельца проекта либо при конкретном требовании или претензии.

Это осознанное принятие риска, а не юридическое заключение. Полная формулировка
и границы решения закреплены в
[`ADR 0003`](decisions/0003-licence-risk-is-advisory.md).
