...
Первый CI head `f14e7dc` выявил изменение `features` в старом JSON/STL test.
Первоначальная review-поправка теста в `640e43a` считала причину отказа Cut
контекстной. Последующий native CI обнаружил тот же эффект при добавлении H/V:
это регрессия каталога для **NewBody**, а не устаревшее ожидание тестов.

Исправлена причина: общий каталог и точечный `saved_cut` вызывают одну проверку
операции до диагностики всей истории. NewBody снова стабильно направляет в
`edit-extrude`; настоящие Cut получают отказы проверенной истории. История
по-прежнему читается один раз. В JSON/STL test восстановлено полное равенство
`features`, дополнительная сверка с pinned каталогом сохранена. Новый document
regression проверяет прямоугольный и наклонный профиль, с одним и двумя Body.
Оба неуспешных CI head не считаются проходом; окончательное исправление требует
нового полного CI по точному SHA.

После исправления маршрутизации повторены native: 22 document cut tests,
68 CLI tests (JSON, ограничения, Circle и прежние copy editors), без skips;
workspace clippy all-targets/all-features и fmt. Genuine stub debug: 22 document
и 12 JSON tests исполнены, один native JSON gate явно skipped; CMakeCache
NOTFOUND и отсутствие libTK/PlaneGCS в imports свежего CLI подтверждены.
Пробная release-конфигурация того же stub target обнаружила Homebrew OCCT;
она остановлена до запуска CLI и не зачтена. Использован прежний проверенный
debug target, без изменения исходников или чужих установок.
